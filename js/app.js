...
// Guard: show user-visible error if config fetch fails instead of silently hanging
let cfg;
try {
  cfg = await _cfgPromise;
} catch (configErr) {
  console.error('[FeeTracker] Config load failed:', configErr);
  // Reveal login screen and show a clear, actionable error message
  ...
  if (loginCont) {
    ...
    // Insert error notice above the button
    const note = loginCont.querySelector('.login-note');
    if (note) note.insertAdjacentHTML('beforebegin', '<div style="display:flex;align-items:center;gap:8px;background:rgba(255,77,109,.1);border:1px solid rgba(255,77,109,.3);border-radius:10px;padding:10px 14px;margin-bottom:16px;font-size:13px;color:#ff4d6d;"><svg width='14' height='14' viewBox='0 0 14 14' fill='none'><path d='M7 2L1 12h12L7 2z' stroke='currentColor' stroke-width='1.4' stroke-linejoin='round'/><line x1='7' y1='6' x2='7' y2='9' stroke='currentColor' stroke-width='1.4' stroke-linecap='round'/><circle cx='7' cy='11' r='.7' fill='currentColor'/></svg>App config unavailable — check your connection and try again.</div>');
  }
  throw configErr; // stop module execution; event listeners below won't attach until retry
}
