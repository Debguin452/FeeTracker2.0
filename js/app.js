...
// Analytics dashboard (charts) is lazy-loaded from js/features/dashboard.js —
// only fetched when a user actually opens it, not on every app boot.
// These stubs load the module once (memoized) then replace themselves with
// the real implementations and re-invoke, so every existing call site
// (onclick markup, dashBtn listener, sbGoAnalytics, handleTotalCardClick,
// the screen-stack back handler) keeps working untouched.
let _dashboardModulePromise = null;
function _loadDashboardModule(){
  if(!_dashboardModulePromise){
    _dashboardModulePromise = import('./features/dashboard.js').then(({initDashboard})=>{
      initDashboard({
        getBatches:        () => batches,
        getTeachers:       () => teachers,
        getPayments:       () => payments,
        getStandaloneCount:() => (_standaloneStudents||[]).length,
        stuCol, bpyCol, getDocs,
        addM, mBetween, monthsDue, calcDue,
        dashFmt, isDarkMode, USER_LOCALE,
        destroyCharts, tchDashCharts:_tchDashCharts, stuDashCharts:_stuDashCharts,
        screenFadeTo, sbSetPage,
      });
    });
  }
  return _dashboardModulePromise;
}
window.openTeacherDash  = async (...a)=>{ await _loadDashboardModule(); return window.openTeacherDash(...a); };
window.openStudentDash  = async (...a)=>{ await _loadDashboardModule(); return window.openStudentDash(...a); };
window.closeTeacherDash = async (...a)=>{ await _loadDashboardModule(); return window.closeTeacherDash(...a); };
window.closeStudentDash = async (...a)=>{ await _loadDashboardModule(); return window.closeStudentDash(...a); };
window.renderStudentDash= async (...a)=>{ await _loadDashboardModule(); return window.renderStudentDash(...a); };
